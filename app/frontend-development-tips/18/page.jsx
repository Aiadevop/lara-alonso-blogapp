    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                // Verificar si la respuesta contiene un error de Marvel API
                if (result.code === "InvalidCredentials" || result.error) {
                    throw new Error(result.message || result.error || 'Error en la API de Marvel');
                }
            ...
            <article className="max-w-5xl px-6 py-24 mx-auto space-y-12 bg-gray-100 text-gray-900 mt-20">
                ...
                {loading && (
                    <div className="text-center py-8">
                        <div className="inline-block animate-spin rounded-full h-8 w-8 border-b-2 border-violetitle"></div>
                        <p className="mt-2">Cargando datos de Marvel...</p>
                    </div>
                )}
                ...
                {data && data.data && data.data.results ? (
                            ...
                            {data.data.results.map((comic, index) => (
                                !comic.thumbnail.path.includes('not_available') && (
                                    <a key={comic.id} href={comic.urls[index % comic.urls.length]?.url} target="blank">
                                        <div className="comic-item bg-neutral-300 min-h-[260px] min-w-[200px] mb-7 border border-solid border-blue-700 rounded-lg hover:shadow-lg transition-shadow">
                                            <img
                    ...
                    <div className="text-center py-8">
                        <p>No se encontraron datos de cómics.</p>
                    </div>
                )}
